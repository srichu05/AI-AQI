...
export type TargetSample = {
  ...
  y: number; // 0 to 1 normalized canvas Y (0 = top, 1 = bottom)
};
...
export type CloudShaderProps = {
  ...
  /** Target coordinates for sampling cloud brightness */
  samples?: TargetSample[];
  /** Callback fired when brightness state changes at target samples */
  onBrightnessResult?: (results: Record<string, boolean>) => void;
};
...
// Cloud shader for Aceternity UI.
const FRAG = `
...
export const CloudShader = ({
  ...
  const onBrightnessResultRef = useRef(onBrightnessResult);
  const stateMapRef = useRef<Record<string, boolean>>({});

  samplesRef.current = samples;
  onBrightnessResultRef.current = onBrightnessResult;
  ...
  useEffect(() => {
    ...
    // Instant cloud detection thresholds:
    // Normal blue sky is ~180.
    // Front edge of cloud is >= 200.
    const HIGH_THRESHOLD = 200;
    const LOW_THRESHOLD = 188;
    ...
    const draw = (now: number) => {
      ...
      // Perform instant localized WebGL pixel sampling directly underneath elements
      if (
        samplesRef.current &&
        samplesRef.current.length > 0 &&
        onBrightnessResultRef.current
      ) {
        const curSamples = samplesRef.current;
        let changed = false;

        for (let i = 0; i < curSamples.length; i++) {
          const s = curSamples[i];
          const px = Math.min(
            canvas.width - 1,
            Math.max(0, Math.floor(s.x * canvas.width)),
          );
          const py = Math.min(
            canvas.height - 1,
            Math.max(0, Math.floor((1.0 - s.y) * canvas.height)),
          );
          ...
          const prev = !!stateMapRef.current[s.id];
          let next = prev;
          if (!prev && lum >= HIGH_THRESHOLD) {
            next = true;
          } else if (prev && lum < LOW_THRESHOLD) {
            next = false;
          }

          if (next !== prev) {
            stateMapRef.current[s.id] = next;
            changed = true;
          }
        }

        if (changed) {
          onBrightnessResultRef.current({ ...stateMapRef.current });
        }
      }
